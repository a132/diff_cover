...
class DiffViolations(object):
    ...
    """
    def __init__(self, violations, measured_lines, diff_lines, branch_lines):
        self.lines = {
...
class BaseReportGenerator(object):
    ...
    def percent_branch_covered(self):
        """
        Return a float percent of lines covered for the source
        in `src_path`.
        ...
        If we have no coverage information for `src_path`, returns None
        """
        diff_violations = self._diff_violations().values()

        if diff_violations is None:
            return None
        total_branch = 0
        cover_branch = 0
        for key, diff_violation in self._diff_violations().items():
            print(key)
            branch_lines = diff_violation.branch_lines
            if not branch_lines:
                continue
    ...
    def violation_branch_lines(self, src_path):
        """
        Return a list of lines in violation (integers)
        in `src_path` that were changed.
        ...
        if not self._diff_violations_dict:
            self._diff_violations_dict = {
                    src_path: DiffViolations(
                        ...
                        self._violations.branch_lines(src_path),
                    )
...
class TemplateReportGenerator(BaseReportGenerator):
    ...
    def _context(self):
        ...
        return {
            ...
            'total_percent_branch_covered': self.percent_branch_covered(),
    ...
    def _src_path_stats(self, src_path):
        ...
        violation_branch_lines = self.violation_branch_lines(src_path)
        ...
        return {
            ...
            'violation_branch_lines': TemplateReportGenerator.combine_adjacent_lines(violation_branch_lines),
